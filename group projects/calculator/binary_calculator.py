...
def dec_to_bin(dec_num):
    ...
    return (f'converted to binary is {"".join(bin_list)}')
...
def bin_add(bin_input1, bin_input2):
    ...
    if carry > 0:
        result += '1'
    return f'added together is {result[-1::-1]}'
...
def bin_sub(val1, val2):
    ...
    return f'Subracted equals {clear0(bin_str)}'
...
def bin_mult(x, y):
   x, y, max_length = equalize_lengths(x, y)

   result = '0'
   for i in range(max_length - 1, -1, -1):
      if y[i] == '1':
         result = bin_add(result, x + ''.zfill(max_length - 1 - i))
   return result
